...
2. لا تكتفِ بشرح ما ستفعله في النص، بل **نفذه** فعلياً عبر الأداة.
3. قدم شرحاً تقنياً مختصراً لما قمت به في حقل \`explanation\` التابع للأداة.
4. إذا كان الطلب يتطلب عدة ملفات، أرسلها جميعاً في استدعاء واحد للأداة.
5. تذكر: أنت تمتلك القدرة على تغيير بيئة العمل (Workspace) لحظياً، وهذا هو دورك المحوري.`;

    const tools = {
      update_workspace_files: tool({
        description: 'استخدم هذه الأداة **دائماً** لإنشاء أو تعديل أي ملف برمجي في بيئة عمل المستخدم. لا تضع الكود في ردك النصي إذا كان يمكنك وضعه في ملف.',
    ...
    // قائمة الموديلات المقترحة للـ Gemini - تم ضبط الأولوية بناءً على دعم الأدوات (v1beta)
    const geminiModels = [
      'gemini-1.5-flash-latest',
      'gemini-2.0-flash',
      'gemini-1.5-pro-latest',
      'gemini-1.5-flash',
      'gemini-2.5-flash-latest', // إبقاء مسمى المستخدم كخيار أخير إذا كان مدعوماً في بيئتهم
    ];
