...
export function SmartSidebarItem({ item, activeTab, onTabChange, isCollapsed, isBeta }: SmartSidebarItemProps) {
  const [isOpen, setIsOpen] = React.useState(false);
  const { isLeft, flexDir, protectivePadding } = useSidebarLayout();
    ...
    <SidebarMenuItem onContextMenu={handleRightClick}>
      <DropdownMenu open={isOpen} onOpenChange={setIsOpen} dir="rtl">
        ...
        >
            ...
            />
            {!isCollapsed && (
              <div className={cn("flex items-center gap-2 flex-1 overflow-hidden relative", protectivePadding)}>
