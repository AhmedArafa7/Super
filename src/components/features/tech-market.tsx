...
import { 
  ...
  Upload, Trash2, Wallet, Loader2, Plus, ShoppingBag
} from "lucide-react";
...
export function TechMarket() {
  ...
  const handleBuyNow = async (item: MarketItem) => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Sync Error", description: "Payment link failed." });
    } finally {
      setIsProcessing(false);
    }
  };
    ...
    <div className="p-8 max-w-7xl mx-auto flex flex-col min-h-full">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-lg">Decentralized exchange for neural digital assets.</p>
        </div>
        ...
        <div className="flex items-center gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
          ...
          <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
            <DialogTrigger asChild>
              <Button className="bg-primary rounded-xl px-6 h-12 shadow-lg shrink-0">
                <Plus className="mr-2 size-5" /> Listing
              </Button>
            </DialogTrigger>
            <DialogContent className="bg-slate-900 border-white/10 rounded-3xl">
              ...
              <div className="grid gap-4 py-4">
                <div className="grid gap-1.5">
                   <Label>Title</Label>
                   <Input placeholder="Asset Name" value={newListing.title} onChange={e => setNewListing({...newListing, title: e.target.value})} className="bg-white/5 border-white/10" />
                </div>
                <div className="grid gap-1.5">
                   <Label>Description</Label>
                   <Textarea placeholder="Functional specifications..." value={newListing.description} onChange={e => setNewListing({...newListing, description: e.target.value})} className="bg-white/5 border-white/10" />
                </div>
              ...
              <DialogFooter>
                <Button onClick={async () => { await addMarketItem({ ...newListing, ownerId: user!.id, ownerName: user!.name }); setIsAddModalOpen(false); loadData(); }} className="w-full bg-primary h-12 rounded-xl">Publish to Nexus</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="md:col-span-3 relative">
          ...
          <Input 
            placeholder="Scan for assets (e.g., 'Quantum', 'GPU', 'Neural Link')..." 
            className="w-full h-14 bg-white/5 border-white/10 rounded-2xl pl-12 text-lg focus-visible:ring-primary"
            value={search}
        ...
        <div className="relative">
          ...
          <Select value={category} onValueChange={(v: any) => setCategory(v)}>
            <SelectTrigger className="h-14 bg-white/5 border-white/10 rounded-2xl pl-10 focus-visible:ring-primary">
      ...
      <div className="flex-1">
        {isLoading && items.length === 0 ? (
          ...
          <EmptyState 
            ...
            description="No neural assets matching your filters were found in this sector."
            action={
...
function ProductCard({ item, onBuy, isProcessing, isMine }: { item: MarketItem, onBuy: (i: MarketItem) => void, isProcessing: boolean, isMine: boolean }) {
  return (
    <Card className={cn(
      "group glass rounded-[2.5rem] overflow-hidden border-white/5 hover:border-primary/40 transition-all duration-500 flex flex-col h-full",
      ...
      <div className="relative aspect-square overflow-hidden bg-white/5">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          <Badge className={cn("backdrop-blur-md border-white/10", item.listingType === 'sell_offer' ? "bg-indigo-600/60" : "bg-amber-600/60")}>
            {item.listingType === 'sell_offer' ? 'SALE' : 'WANTED'}
          </Badge>
          <Badge className="bg-black/40 backdrop-blur-md border-white/10 text-[10px] uppercase font-bold">
      ...
      <CardContent className="p-6 flex flex-col flex-1">
        ...
        <div className="mt-auto pt-6 border-t border-white/5 flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xl font-bold text-white tracking-tighter">{item.price} <span className="text-xs text-muted-foreground uppercase font-normal">{item.currency}</span></p>
          </div>
          <div className="text-right">
            <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Inventory</p>
        ...
        {isMine && (
          <div className="w-full h-12 border border-dashed border-white/10 rounded-xl flex items-center justify-center text-[10px] text-muted-foreground uppercase font-bold">
            Own Node Asset
          </div>
        )}
      </CardContent>
