...
function Sidebar({ isSidebarOpen, activeTab, setActiveTab, subscriptions }: any) {
  ...
  if (!isSidebarOpen) {
    return (
      <aside className="hidden md:flex w-24 flex-col items-center py-4 gap-6 bg-black/20 backdrop-blur-md rounded-3xl border border-white/5 h-full rtl overflow-y-auto no-scrollbar shrink-0">
        {mainLinks.map(link => (
          <button
            key={link.id} onClick={() => setActiveTab(link.id)}
            className={cn(
              "flex flex-col items-center justify-center w-20 gap-2 p-3 rounded-2xl transition-all duration-300",
              activeTab === link.id ? "bg-white/10 text-white shadow-lg" : "text-white/60 hover:bg-white/5 hover:text-white"
            )}
          >
            <link.icon className={cn("size-7 font-light", activeTab === link.id && "fill-current")} />
            <span className="text-[11px] font-medium truncate max-w-full px-1">{link.label}</span>
          </button>
        ))}
        <button
          ...
          className={cn(
            "flex flex-col items-center justify-center w-20 gap-2 p-3 rounded-2xl transition-all duration-300",
            activeTab === 'library' ? "bg-white/10 text-white shadow-lg" : "text-white/60 hover:bg-white/5 hover:text-white"
          )}
        >
          <History className="size-7 font-light" />
          <span className="text-[11px] font-medium">المكتبة</span>
        </button>
      </aside>
    );
  }
  ...
  return (
    <aside className="hidden md:flex w-64 flex-col py-4 bg-black/20 backdrop-blur-md rounded-3xl border border-white/5 h-full hover:overflow-y-auto no-scrollbar rtl text-right shrink-0">
        ...
        {subscriptions.length === 0 ? (
          ...
          subscriptions.map((sub: any) => (
            <button key={sub.id} onClick={() => setActiveTab('subs')} className="flex items-center gap-4 w-full p-2 px-3 rounded-xl hover:bg-white/10 transition-colors">
              ...
              <span className="text-sm truncate">{sub.channelTitle || "قناة غير معروفة"}</span>
