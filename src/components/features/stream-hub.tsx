        ...
        {(activeView === 'explore' ? publicVideos : safeVideos.filter(v => v.authorId === user?.id)).map((video) => (
          ...
          >
            <div className="relative aspect-video overflow-hidden bg-slate-900">
              <Image 
                src={video.source === 'youtube' ? `https://img.youtube.com/vi/${video.externalUrl?.split('v=')[1]?.split('&')[0]}/maxresdefault.jpg` : video.thumbnail || "https://images.unsplash.com/photo-1611162617474-5b21e879e113?q=80&w=1074&auto=format&fit=crop"} 
                alt={video.title} 
                fill
