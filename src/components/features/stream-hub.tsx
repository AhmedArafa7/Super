...
import { Play, Plus, Upload, MoreVertical, Clock, Eye, Trash2, ShieldCheck, Lock, EyeOff, CheckCircle2, AlertCircle, LayoutDashboard, Globe, Loader2, X, Zap, Share2, MessageSquare, Youtube, ExternalLink, FileVideo } from "lucide-react";
...
export function StreamHub() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      toast({
        ...
        description: "File exceeds 1.5MB Nexus limit. Use YouTube for longer videos.",
      });
      return;
    }
  ...
  const handleFinalizeUpload = () => {
    ...
    if (uploadSource === 'youtube' && !uploadData.externalUrl) return;
    ...
    setTimeout(async () => {
      ...
      try {
        await addVideo({
          ...
          time: uploadSource === 'youtube' ? "External" : "0:45",
          status: isAdmin ? 'published' : 'pending_review',
          visibility: uploadData.visibility,
          allowedUserIds: allowedUserList,
          uploaderRole: user.role,
          source: uploadSource,
          externalUrl: uploadSource === 'youtube' ? uploadData.externalUrl : undefined
        });
        ...
        setTimeout(() => {
          ...
          setUploadData({ title: "", visibility: "public", allowedUsers: "", thumbnail: "", externalUrl: "", fileSize: 0 });
  ...
  const handleVideoClick = (video: Video) => {
    if (video.source === 'youtube' && video.externalUrl) {
      window.open(video.externalUrl, '_blank');
      toast({ title: "Redirecting...", description: "Opening neural broadcast on YouTube." });
    } else {
            ...
            <DialogContent className="sm:max-w-[550px] bg-slate-950 border-white/10 rounded-[2.5rem] p-8 shadow-[0_0_50px_rgba(99,102,241,0.1)]">
              <DialogHeader>
                ...
                <DialogDescription className="text-muted-foreground text-sm">
                  Neural payloads are scanned for protocol compliance. Choose between local clips or YouTube broadcasts.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-6 py-6">
                <Tabs value={uploadSource} onValueChange={(v: any) => setUploadSource(v)} className="w-full">
                  <TabsList className="grid w-full grid-cols-2 bg-white/5 p-1 rounded-xl">
                    <TabsTrigger value="local" className="rounded-lg gap-2"><FileVideo className="size-4" /> Local Clip</TabsTrigger>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2">
                    <Label className="text-xs uppercase font-bold tracking-widest text-muted-foreground px-1">Thumbnail Payload</Label>
                    <div className="relative h-12 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center px-4 cursor-pointer hover:bg-white/10 transition-colors overflow-hidden">
                      ...
                      {uploadData.thumbnail ? <span className="text-[10px] text-green-400 font-bold truncate">Visual Logged</span> : <span className="text-[10px] text-muted-foreground font-bold">Scan Image</span>}
                    </div>
                  </div>
                </div>
              ...
              <DialogFooter>
                <Button 
                  ...
                  disabled={!uploadData.title || (uploadSource === 'local' && !uploadData.thumbnail) || (uploadSource === 'youtube' && !uploadData.externalUrl) || isUploading}
                >
                  {isUploading ? <Loader2 className="mr-2 size-5 animate-spin" /> : <Zap className="mr-2 size-5" />}
                  Authorize Broadcast
                </Button>
              </DialogFooter>
            </DialogContent>
        ...
        {(activeView === 'explore' ? publicVideos : mySubmissions).map((video) => (
          ...
          >
            <div className="relative aspect-video overflow-hidden">
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500">
                <div className="size-20 bg-primary/20 backdrop-blur-md rounded-full flex items-center justify-center border border-white/20 shadow-2xl scale-75 group-hover:scale-100 transition-transform duration-500">
                  {video.source === 'youtube' ? <ExternalLink className="text-white size-10" /> : <Play className="text-white size-10 fill-white ml-1.5" />}
                </div>
              </div>

              <div className="absolute top-5 left-5 flex gap-2">
                 <Badge className="bg-black/60 backdrop-blur-md border-white/10 flex items-center gap-1.5 py-1.5 px-3 rounded-xl">
                    {video.source === 'youtube' ? <Youtube className="size-3 text-red-500" /> : <Globe className="size-3" />}
                    <span className="text-[10px] font-bold uppercase tracking-widest">{video.source === 'youtube' ? 'YouTube' : video.visibility}</span>
                 </Badge>
              </div>
            ...
            <div className="p-8 flex flex-col flex-1">
              <h3 dir="auto" className="font-bold text-2xl leading-tight text-white/90 group-hover:text-primary transition-colors line-clamp-2 text-right mb-6 w-full">{video.title}</h3>
              ...
              <div className="mt-auto flex items-center justify-between pt-6 border-t border-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground flex items-center gap-1 uppercase font-bold mt-1">
                      <Eye className="size-3 text-primary" /> {video.views} Links
                    </p>
                  </div>
                </div>
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="size-10 rounded-xl text-muted-foreground hover:bg-red-500/10 hover:text-red-400" 
        ...
        <DialogContent className="max-w-6xl bg-black border-white/10 p-0 overflow-hidden rounded-[3rem] shadow-[0_0_100px_rgba(99,102,241,0.3)]">
          <div className="relative aspect-video bg-slate-900 group">
            ...
            <Button 
              ...
              className="absolute top-6 right-6 z-50 bg-black/40 backdrop-blur-md rounded-full border border-white/10 text-white"
            >
              <X className="size-6" />
            </Button>
          </div>
          <div className="p-10 bg-slate-950">
            <div className="flex flex-col lg:flex-row gap-8 justify-between items-start">
              <div className="flex-1 space-y-4">
                ...
                <div className="flex items-center gap-6 text-muted-foreground pt-2 justify-end font-bold">
                  <div><Eye className="size-4 text-primary inline mr-2" /> {selectedVideo?.views} Links</div>
                  <div><Clock className="size-4 text-primary inline mr-2" /> {selectedVideo && new Date(selectedVideo.createdAt).toLocaleDateString()}</div>
                </div>
              </div>
              <div className="flex items-center gap-4 bg-white/5 p-6 rounded-[2rem] border border-white/10 w-full lg:w-auto">
                <img src={`https://picsum.photos/seed/${selectedVideo?.author}/64/64`} className="size-14 rounded-2xl object-cover shadow-2xl" />
                <div className="flex-1"><p className="font-bold text-white text-lg">@{selectedVideo?.author}</p><p className="text-xs text-muted-foreground uppercase tracking-widest font-bold">Authorized Node</p></div>
