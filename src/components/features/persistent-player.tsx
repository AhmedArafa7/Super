...
export const PersistentPlayer = memo(() => {
  ...
  const shouldShow = useMemo(() => {
    ...
    if (isAtHome) return false; // لا نظهر المشغل المستمر في صفحة Stream لأن WeTubeWatchView يتكفل بالعرض
    return autoFloat;
  }, [activeVideo, isAtHome, autoFloat]);

  // إخفاء المشغل تماماً إذا لم يكن هناك فيديو أو كنا في صفحة 
  // WeTube الأصلية لكي لا يحدث تكرار في التشغيل
  if (!activeVideo || isAtHome) return null;
  ...
  return (
    ...
    >
      <div className={cn(
        ...
        "opacity-0 group-hover/mini:opacity-100 bg-gradient-to-b from-black/80 to-transparent"
      )}>
      ...
      {shouldShow && (
        <div
          className="absolute inset-0 z-10 flex items-center justify-center bg-black/40 opacity-0 group-hover/mini:opacity-100 transition-opacity cursor-pointer pointer-events-none"
        >
        </div>
      )}
    </div>
