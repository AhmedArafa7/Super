...
/**
 * @fileOverview المشغل العصبي المستمر - يدعم التنقل الحر في الموقع دون انقطاع البث.
 */
export const PersistentPlayer = memo(() => {
  const { activeVideo, isPlaying, isMinimized, quality, setActiveVideo, setIsPlaying, setIsMinimized } = useStreamStore();

  if (!activeVideo) return null;

  const ytId = activeVideo.source === 'youtube' ? getYoutubeId(activeVideo.externalUrl) : null;
    ...
    <div 
      className={cn(
        ...
        isMinimized 
          ...
          : "inset-x-4 bottom-4 top-20 md:inset-x-20 md:bottom-10 md:top-24 rounded-[2.5rem]"
      )}
    >
      {/* Header Controls */}
      <div className={cn(
        "absolute top-0 inset-x-0 p-4 z-20 flex items-center justify-between transition-opacity duration-300",
        isMinimized ? "opacity-0 group-hover/mini:opacity-100 bg-gradient-to-b from-black/80 to-transparent" : "bg-gradient-to-b from-black/60 to-transparent"
      )}>
        <div className="flex items-center gap-2">
          ...
          {!isMinimized && (
            <span dir="auto" className="text-white text-sm font-bold truncate max-w-[200px]">
              {activeVideo.title}
            </span>
          )}
        </div>
      ...
      {/* Mini-Overlay UI for small mode */}
      {isMinimized && (
        <div 
