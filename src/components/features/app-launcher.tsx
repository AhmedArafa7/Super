      ...
      <div className="flex flex-col h-full bg-black animate-in fade-in duration-500">
        <header className="h-16 border-b border-white/5 bg-slate-900/80 backdrop-blur-xl flex items-center justify-between px-6 z-20 flex-row-reverse">
          <div className="flex items-center gap-4 flex-row-reverse">
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest flex items-center gap-2 justify-end">
                <span className="size-1.5 rounded-full bg-green-500 animate-pulse" /> Live Production Node
              </p>
            </div>
          </div>
          <Button variant="outline" size="sm" className="h-9 rounded-xl border-white/10 text-[10px] font-bold bg-white/5 hover:bg-primary" onClick={() => window.open(activeProject.url, '_blank')}>
            <ExternalLink className="size-3 mr-1.5" /> زيارة الموقع الأصلي
          </Button>
        </header>
        <div className="flex-1 relative bg-white">
          <iframe src={activeProject.url} className="absolute inset-0 size-full border-none" title={activeProject.title} sandbox="allow-scripts allow-same-origin allow-forms allow-popups" />
        </div>
      </div>
    );
  }
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-12 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 flex-row-reverse text-right">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-lg">شغل تطبيقاتك السحابية مباشرة. حدود الاستهلاك تخضع للمراجعة الإدارية.</p>
        </div>
