...
import { X, Download, ExternalLink, FileText, Image as ImageIcon, Video as VideoIcon, Music, Edit3, Check, Trash2, Palette, Settings2, Loader2, Save } from "lucide-react";
...
/**
 * [STABILITY_ANCHOR: VAULT_PREVIEW_V2.0]
 * مودال المعاينة المتقدم - يدعم التحكم الكامل: إعادة التسمية، تخصيص المظهر، وإجراءات الملف.
 */
export function VaultPreviewModal({ asset, onClose, onRefresh }: VaultPreviewModalProps) {
  ...
  return (
    <Dialog open={!!asset} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-6xl bg-slate-950 border-white/10 p-0 overflow-hidden rounded-[2.5rem] shadow-[0_0_80px_rgba(0,0,0,0.9)] focus:outline-none flex flex-col md:flex-row h-[85vh]">
        ...
        <aside className="w-full md:w-72 border-r border-white/5 bg-slate-900/40 p-6 flex flex-col gap-8 order-2 md:order-1">
          <div className="space-y-4">
            ...
            <div className="grid gap-2">
              ...
              >
                <Edit3 className="size-4 text-amber-400" /> إعادة التسمية
              </Button>
              <Button 
                variant="outline" 
             ...
             <div className="flex items-center gap-4 flex-row-reverse text-right flex-1 min-w-0">
                {isEditingName ? (
                  ...
                  <div className="flex flex-col min-w-0">
                    <DialogTitle dir="auto" className="text-white font-bold text-lg truncate pr-2 tracking-tight group flex items-center gap-3 justify-end">
